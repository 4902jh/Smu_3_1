...
void insertLastNode(linkedList_h* L, char* x) {
    ...
    temp->link = newNode;    // 새 노드를 마지막 노드 뒤에 연결
}
...
void deleteNode(linkedList_h* L, listNode* p) {
    ...
    if (L->head->link == NULL) {  // 리스트에 노드가 한 개만 있는 경우
    ...
    else if (p == NULL) return;   // 삭제할 노드가 없다면 중단
    else {
        pre = L->head;
        ...
        pre->link = p->link;  // 선행자 노드와 다음 노드를 연결
        free(p);              // 삭제 노드 메모리 해제
    }
}
